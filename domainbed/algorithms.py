...
class Clip_train_feature_mixup_domainwise_text_mix_cls(Clip_train_mixup_with_text):
    def update(self, minibatches, unlabeled=None):
        # start_time=time.time()
        all_x = torch.cat([x for x,y in minibatches])
        all_y = torch.cat([y for x,y in minibatches])
        ...
        for dom in range(self.num_domains-1):
            ...
            loss_mix+=0.5*torch.mean(F.cross_entropy(logits_per_image_mixup, mixup_labels_all[dom][0],reduction='none')*a[:,dom+1],dim=0)
            loss_mix+=0.5*torch.mean(F.cross_entropy(logits_per_image_mixup, rand_perm[mixup_labels_all[dom][0].to("cpu")].to("cuda"),reduction='none')*a[:,dom+1],dim=0)
        loss=loss_ce+(1/self.num_domains)*loss_mix
        # print(loss_ce2)
        # logits_per_text_mixup = logits_per_image_mixup.t()
...
class Clip_train_manifold_mixup_with_text(Clip_train_mixup_with_text):
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        for i in range(self.num_mixups):
            ...
            mixup_text_feature=torch.unsqueeze(a[:,0],dim=1).expand(-1,512)*mixup_text_feature
            rand_perm=torch.randperm(bs)
            cfg={'manifold':True,'mixup_weights':a,'ndom':self.num_domains,'mix_layer':1,'domainwise':True,'post':False,'rand_perm':rand_perm}
...
class Clip_train_multi_modal_manifold_mixup(Clip_train_mixup_with_text):
    """
    Empirical Risk Minimization (ERM)
    """
    def update(self, minibatches, unlabeled=None):
...
class Clip_train_feature_mixup_clswise(Clip_train_mixup_with_text):
    """
    Empirical Risk Minimization (ERM)
    """
    def update(self, minibatches, unlabeled=None):
