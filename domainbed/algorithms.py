...
class Clip_train_feature_mixup_domainwise_text_mix_cls(Clip_train_mixup_with_text):
    def update(self, minibatches, unlabeled=None):
        ...
        loss_mix=0.5*torch.mean(F.cross_entropy(logits_per_image_mixup, all_y,reduction='none')*a[:,0],dim=0)
        loss_mix+=0.5*torch.mean(F.cross_entropy(logits_per_image_mixup, rand_perm[all_y.to("cpu")].to("cuda"),reduction='none')*a[:,0],dim=0)
