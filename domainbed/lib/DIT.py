...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.,attn_sep_mask=False):
        super().__init__()
    ...
    def forward(self, x):
        ...
        if(self.attn_sep_mask):
            list_ind=[0,N-1] # No dist token is considered _ if dist token included 0,1,N-1
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm,attn_sep_mask=False):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop,attn_sep_mask=attn_sep_mask)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 act_layer=None, weight_init='',add_di_token=False,attn_sep_mask=False,no_embeddings=False):
        """
        Args:
        ...
        self.di_token=nn.Parameter(torch.zeros(1, 1, embed_dim)) if add_di_token else None
        self.pos_embed = nn.Parameter(torch.zeros(1, num_patches + self.num_tokens, embed_dim))
        self.pos_embed2=nn.Parameter(torch.zeros(1,1 , embed_dim)) if add_di_token else None

        # print(self.pos_embed2.shape)
        ...
        self.blocks = nn.Sequential(*[
            Block(
                ...
                attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer,attn_sep_mask=attn_sep_mask)
    ...
    def forward_features(self, x):
        if not (self.no_embeddings):
            ...
            x=einops.rearrange(x, 'b N h w -> b h w N').reshape(x.shape[0],-1,self.embed_dim)
        cls_token = self.cls_token.expand(x.shape[0], -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
        if self.dist_token is None and self.di_token is None:
            x = torch.cat((cls_token, x), dim=1)
        elif self.dist_token is not None and self.di_token is None:
            x = torch.cat((cls_token, self.dist_token.expand(x.shape[0], -1, -1), x), dim=1)
        elif self.dist_token is None and self.di_token is not None:
            x = torch.cat((cls_token, x,self.di_token.expand(x.shape[0], -1, -1)), dim=1)
        else:
            x = torch.cat((cls_token, self.dist_token.expand(x.shape[0], -1, -1), x,self.di_token.expand(x.shape[0], -1, -1)), dim=1)
        # if not (self.no_embeddings):
        x = self.pos_drop(x + self.pos_embed) if self.di_token is None else self.pos_drop(x + torch.cat([self.pos_embed,self.pos_embed2],dim=1))
    ...
    def forward(self, x,return_cls_feat=False,class_di_add=False):
        ...
        if self.di_token is not None:
            if self.head_dist is not None:
                x, x_dist,di_tok = self.head(x[0]), self.head_dist(x[1]),x[-1] # x must be a tuple
                if self.training and not torch.jit.is_scripting():
                    # during inference, return the average of both classifier predictions
                    return x, x_dist,di_tok
                else:
                    return (x + x_dist) / 2
