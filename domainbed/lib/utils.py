...
def get_model(args, pretrained=True):
    ...
    if args.model_name in model_names:
        ...
        algorithm_class = algorithms.get_algorithm_class(model1["args"]["algorithm"])
...
def parse_args():
    ...
    parser.add_argument('--generate_images_blockwise', action='store_true', default=False, help="generate images blockwise instead of eval")
    parser.add_argument('--domain', type=str, default=None, help='domain name')
    parser.add_argument('--jacard_out', type=str, default="./jaccard_out", help='domain name')
    parser.add_argument('--generate_images_block_asbatch', action='store_true', default=False, help="use shape token for prediction")
