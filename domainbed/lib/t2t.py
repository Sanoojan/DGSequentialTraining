...
from.t2t_utils import load_for_transfer_learning
...
class T2T_ViT(nn.Module):
    def __init__(self, img_size=224, tokens_type='performer', in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., norm_layer=nn.LayerNorm, token_dim=64,add_di_token=False,attn_sep_mask=False):
        super().__init__()
        ...
        self.pos_embed2=nn.Parameter(data=get_sinusoid_encoding(n_position=num_patches + 2, d_hid=embed_dim), requires_grad=False) if add_di_token else None
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer,attn_sep_mask=attn_sep_mask)
    ...
    def forward(self, x):
        if self.di_token is not None:
            x,di_tok = self.forward_features(x)
            x = self.head(x)
            return x,di_tok
        else:
...
@register_model
def t2t_vit_7(pretrained=False,num_classes=1000,add_di_tok=False,attn_sep_mask=False, **kwargs): # adopt performer for tokens to token
    if pretrained:
        kwargs.setdefault('qk_scale', 256 ** -0.5)
    model = T2T_ViT(tokens_type='performer', embed_dim=256, depth=7, num_heads=4, mlp_ratio=2.,add_di_token=add_di_tok,attn_sep_mask=attn_sep_mask, **kwargs)
    model.default_cfg = default_cfgs['T2t_vit_7']
    if pretrained:
        load_for_transfer_learning(model,"domainbed/pretrained/T2T/71.7_T2T_ViT_7.pth",use_ema=True, strict=False,num_classes=num_classes)
    return model
...
@register_model
def t2t_vit_t_24(pretrained=False,num_classes=1000,add_di_tok=False,attn_sep_mask=False, **kwargs):  # adopt transformers for tokens to token
    if pretrained:
        kwargs.setdefault('qk_scale', 512 ** -0.5)
    model = T2T_ViT(tokens_type='transformer', embed_dim=512, depth=24, num_heads=8, mlp_ratio=3.,add_di_token=add_di_tok,attn_sep_mask=attn_sep_mask, **kwargs)
    model.default_cfg = default_cfgs['T2t_vit_t_24']
    if pretrained:
        load_for_transfer_learning(model,"domainbed/pretrained/T2T/82.6_T2T_ViTt_24.pth",use_ema=True, strict=False,num_classes=num_classes)
    return model
