...
def generate_featurelized_loader(loader, network, classifier, batch_size=32):
    ...
    for x, y in loader:
        ...
        image_features = network.encode_image(x)
        ...
        image_features = image_features @ network.visual.proj

        z = image_features / image_features.norm(dim=1, keepdim=True)
        text_features = text_features / text_features.norm(dim=1, keepdim=True)
    

        # cosine similarity as logits
        logit_scale = network.logit_scale.exp()

        p = logit_scale * z @ text_features.t()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_dirs', type=str,default="/share/users/sanoojan/DGST/domainbed/outputs_clip/Evaluation/PACS/lr-0.000005/")
    parser.add_argument('--adapt_algorithm', type=str, default="T3A")
    args_in = parser.parse_args()
    for i in os.listdir(args_in.input_dirs):
        if("txt" in i):
            continue
        args_in.input_dir=args_in.input_dirs+i+"/"
        # print(os.listdir(args_in.input_dir))
        ...
        lists=os.listdir(args.output_dir)
        model_name='IID_best.pkl'
        for mod_name in lists:
            if "best" in mod_name:
                model_name=mod_name
                break
        assert os.path.exists(os.path.join(args.output_dir, model_name))  # IID_best is produced by train.py
        ...
        if args.dataset in vars(datasets):
            args.data_dir='/share/data/drive_2/DG/data'
        ...
        algorithm_class = algorithms.get_algorithm_class("Testing")
        algorithm = algorithm_class(dataset.input_shape, dataset.num_classes,
            len(dataset) - len(args.test_envs), hparams,args.input_dir)
        ...
        # algorithm=load_model(args.output_dir+'IID_best.pkl')
        # Evaluate base model
        ...
        if use_featurer_cache:
            ...
            for name, loader, weights in original_evals:
                ...
                loader1, loader2, ent = generate_featurelized_loader(loader, network=algorithm.featurizer, classifier=algorithm.text_features, batch_size=32)
        ...
        if args.adapt_algorithm in ['T3A']:
            adapt_hparams_dict = {
                'filter_K': [1,5,20,50,100,-1], #Add -1 filter there too
            }
