...
class T3A(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams, algorithm):
        ...
        # self.param = algorithm.featurizer.visual.proj
        self.text_features=algorithm.text_features
        self.text_features=self.text_features / self.text_features.norm(dim=1, keepdim=True)
        # self.classifier.weight = self.param.to("cuda")

        warmup_supports = self.text_features
        self.warmup_supports = warmup_supports
        warmup_prob = self.text_features @ self.warmup_supports
    ...
    def forward(self, x, adapt=False):
        ...
        if adapt:
            ...
            p = logit_scale * z @ self.text_features.t()
    ...
    def reset(self):
        self.supports = self.warmup_supports
        self.labels = self.warmup_labels.data
        self.ent = self.warmup_ent.data
